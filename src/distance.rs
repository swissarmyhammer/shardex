    ...
    fn test_similarity_range_bounds() {
        ...
        for metric in [DistanceMetric::Cosine, DistanceMetric::Euclidean, DistanceMetric::DotProduct] {
            for (a, b) in &test_vectors {
                let similarity = metric.similarity(a, b).unwrap();
                assert!(similarity >= 0.0, "Similarity {} should be >= 0.0 for metric {:?}", similarity, metric);
                assert!(similarity <= 1.0, "Similarity {} should be <= 1.0 for metric {:?}", similarity, metric);
                assert!(!similarity.is_nan(), "Similarity should not be NaN for metric {:?}", metric);
            }
        }
    }
