...
impl IntegrityReport {
    ...
    pub fn generate_summary(&mut self) {
        ...
        let failed_components = self.component_results.values().filter(|r| !r.is_valid).count();
    ...
    fn calculate_priority(corruption: &CorruptionReport) -> u8 {
        match corruption.corruption_type {
            ...
            CorruptionType::DataCorruption => {
                if corruption.severity > 0.8 { 1 } else { 2 }
            }
            CorruptionType::FileTruncation => 2, // High
            CorruptionType::PartialCorruption => {
                if corruption.severity > 0.5 { 3 } else { 4 }
            }
        }
    }
...
impl IntegrityChecker {
    ...
    pub fn discover_components(&mut self) -> Result<(), ShardexError> {
        ...
        self.component_paths.insert(ComponentType::VectorStorage, vector_files);

        // Posting storage files
        let posting_files = self.find_files_by_pattern("**/*.postings")?;
        self.component_paths.insert(ComponentType::PostingStorage, posting_files);

        // WAL segments
        let wal_files = self.find_files_by_pattern("wal/*.log")?;
        self.component_paths.insert(ComponentType::WalSegments, wal_files);

        // Shardex segments
        let shardex_files = self.find_files_by_pattern("centroids/*.shx")?;
        self.component_paths.insert(ComponentType::ShardexSegments, shardex_files);
    ...
    fn find_files_by_pattern(&self, pattern: &str) -> Result<Vec<PathBuf>, ShardexError> {
        ...
        for entry in glob(&pattern_str).map_err(|e| {
            ShardexError::Corruption(format!("Glob pattern error: {}", e))
        })? {
            match entry {
    ...
    pub async fn verify_full_integrity(&mut self) -> Result<IntegrityReport, ShardexError> {
        ...
        for (component_type, file_paths) in &component_paths {
            let component_result = self.verify_component_integrity(*component_type, file_paths).await?;
    ...
    async fn verify_component_integrity(&mut self, component_type: ComponentType, file_paths: &[PathBuf]) -> Result<ValidationResult, ShardexError> {
        let start_time = Instant::now();
    ...
    async fn verify_vector_storage_checksums(&self, mmf: &MemoryMappedFile) -> Result<(), CorruptionReport> {
        use crate::vector_storage::VectorStorageHeader;

        let header: VectorStorageHeader = mmf.read_at(0)
            .map_err(|e| CorruptionReport {
                corruption_type: CorruptionType::DataCorruption,
        ...
        let vector_data_size = (header.capacity as usize) * (header.vector_dimension as usize) * std::mem::size_of::<f32>();
    ...
    async fn verify_posting_storage_checksums(&self, mmf: &MemoryMappedFile) -> Result<(), CorruptionReport> {
        use crate::posting_storage::PostingStorageHeader;

        let header: PostingStorageHeader = mmf.read_at(0)
            .map_err(|e| CorruptionReport {
                corruption_type: CorruptionType::DataCorruption,
    ...
    fn verify_vector_data_quality(&self, vector_data: &[u8], header: &crate::vector_storage::VectorStorageHeader) -> Result<(), CorruptionReport> {
        ...
        for i in 0..vector_count {
            ...
            if has_nan { nan_vectors += 1; }
            if has_infinite { infinite_vectors += 1; }
            if all_zero { zero_vectors += 1; }
        }
        ...
        if total_vectors > 0 {
            ...
            if nan_ratio > 0.1 {
                return Err(CorruptionReport {
                    ...
                    corruption_offset: None,
                    corruption_size: None,
                    description: format!("Excessive NaN values in vectors: {:.1}% ({}/{})", 
                                       nan_ratio * 100.0, nan_vectors, total_vectors),
            ...
            if infinite_ratio > 0.05 {
                return Err(CorruptionReport {
                    ...
                    corruption_offset: None,
                    corruption_size: None,
                    description: format!("Excessive infinite values in vectors: {:.1}% ({}/{})", 
                                       infinite_ratio * 100.0, infinite_vectors, total_vectors),
                    recovery_recommendations: vec![
            ...
            if zero_ratio > 0.5 {
                return Err(CorruptionReport {
                    ...
                    corruption_offset: None,
                    corruption_size: None,
                    description: format!("Suspicious number of zero vectors: {:.1}% ({}/{})", 
                                       zero_ratio * 100.0, zero_vectors, total_vectors),
                    recovery_recommendations: vec![
    ...
    fn verify_posting_data_quality(&self, _posting_data: &[u8], _header: &crate::posting_storage::PostingStorageHeader) -> Result<(), CorruptionReport> {
    ...
    async fn verify_cross_references(&mut self) -> Result<Vec<CorruptionReport>, ShardexError> {
        ...
        if let (Some(posting_files), Some(vector_files)) = (
            ...
            issues.extend(self.verify_posting_vector_consistency(posting_files, vector_files).await?);
        }
    ...
    async fn verify_posting_vector_consistency(&mut self, posting_files: &[PathBuf], vector_files: &[PathBuf]) -> Result<Vec<CorruptionReport>, ShardexError> {
        let mut issues = Vec::new();
        ...
        for posting_file in posting_files {
            if let Some(shard_id) = self.extract_shard_id_from_path(posting_file) {
                ...
                if let Some(vector_file) = vector_storage_map.get(&shard_id) {
                    ...
                    if let Some(consistency_issue) = self.verify_shard_consistency(posting_file, vector_file).await? {
        ...
        for (shard_id, vector_file) in &vector_storage_map {
            let has_posting_file = posting_files.iter().any(|pf| {
                self.extract_shard_id_from_path(pf).as_ref() == Some(shard_id)
            });
    ...
    async fn verify_shard_consistency(&mut self, posting_file: &Path, vector_file: &Path) -> Result<Option<CorruptionReport>, ShardexError> {
        ...
        if let Err(issue) = self.verify_header_compatibility(&posting_mmf, &vector_mmf).await {
            return Ok(Some(issue));
        }

        // Check capacity and count consistency
        if let Err(issue) = self.verify_capacity_consistency(&posting_mmf, &vector_mmf).await {
            return Ok(Some(issue));
        }
    ...
    async fn verify_header_compatibility(&self, posting_mmf: &MemoryMappedFile, vector_mmf: &MemoryMappedFile) -> Result<(), CorruptionReport> {
        use crate::posting_storage::PostingStorageHeader;
        use crate::vector_storage::VectorStorageHeader;

        // Read headers
        let posting_header: PostingStorageHeader = posting_mmf.read_at(0)
            .map_err(|e| CorruptionReport {
                corruption_type: CorruptionType::CrossValidationFailure,
        ...
        let vector_header: VectorStorageHeader = vector_mmf.read_at(0)
            .map_err(|e| CorruptionReport {
                corruption_type: CorruptionType::CrossValidationFailure,
        ...
        if posting_header.capacity != vector_header.capacity {
            return Err(CorruptionReport {
                ...
                description: format!(
                    ...
                    posting_header.capacity, 
                    vector_header.capacity
                ),
                recovery_recommendations: vec![
        ...
        if vector_header.vector_dimension as usize != expected_vector_size {
            return Err(CorruptionReport {
                ...
                description: format!(
                    ...
                    vector_header.vector_dimension
                ),
                recovery_recommendations: vec![
    ...
    async fn verify_capacity_consistency(&self, posting_mmf: &MemoryMappedFile, vector_mmf: &MemoryMappedFile) -> Result<(), CorruptionReport> {
        use crate::posting_storage::PostingStorageHeader;
        use crate::vector_storage::VectorStorageHeader;

        let posting_header: PostingStorageHeader = posting_mmf.read_at(0)
            .map_err(|_| CorruptionReport {
                corruption_type: CorruptionType::CrossValidationFailure,
                file_path: PathBuf::from("posting_file"),
                corruption_offset: Some(0),
                corruption_size: None,
    ...
    async fn repair_single_issue(&mut self, issue: &IntegrityIssue) -> Result<RepairResult, ShardexError> {
        ...
        let (success, action, notes) = match issue.corruption.corruption_type {
            ...
            _ => (false, "Repair not implemented for this corruption type".to_string(), vec![]),
        };
    ...
    async fn repair_data_corruption(&mut self, corruption: &CorruptionReport) -> Result<(bool, String, Vec<String>), ShardexError> {
        ...
        Ok((false, "Automatic repair not available for this corruption type".to_string(), notes))
    }

    /// Repair checksum mismatches by recalculating and updating checksums
    async fn repair_checksum_mismatch(&mut self, corruption: &CorruptionReport) -> Result<(bool, String, Vec<String>), ShardexError> {
        let mut notes = Vec::new();
        ...
        if corruption.file_path.exists() {
            ...
            if file_data.len() >= FileHeader::SIZE {
                ...
                return Ok((false, "Checksum repair available but not implemented in read-only mode".to_string(), notes));
            }
        }
    ...
    async fn repair_vector_quality_issues(&mut self, corruption: &CorruptionReport) -> Result<(bool, String, Vec<String>), ShardexError> {
        let mut notes = Vec::new();
        ...
        if corruption.description.contains("infinite") {
            ...
            return Ok((false, "Infinite value repair requires vector normalization".to_string(), notes));
        }
    ...
    async fn repair_partial_corruption(&mut self, corruption: &CorruptionReport) -> Result<(bool, String, Vec<String>), ShardexError> {
        let mut notes = Vec::new();
    ...
    async fn repair_cross_validation(&mut self, corruption: &CorruptionReport) -> Result<(bool, String, Vec<String>), ShardexError> {
        let mut notes = Vec::new();
        ...
        Ok((false, "Cross-validation repair not available for this issue type".to_string(), notes))
    }

    /// Repair capacity mismatches between components
    async fn repair_capacity_mismatch(&mut self, _corruption: &CorruptionReport) -> Result<(bool, String, Vec<String>), ShardexError> {
        let mut notes = Vec::new();
        
        notes.push("Capacity mismatch can be resolved by rebuilding one component".to_string());
        notes.push("Choose the component with the correct capacity as the source of truth".to_string());
        notes.push("Rebuild the other component to match the correct capacity".to_string());
        ...
        Ok((false, "Capacity mismatch repair requires component rebuilding".to_string(), notes))
    }

    /// Repair count mismatches between components
    async fn repair_count_mismatch(&mut self, corruption: &CorruptionReport) -> Result<(bool, String, Vec<String>), ShardexError> {
        let mut notes = Vec::new();
        ...
        Ok((false, "Count mismatch repair requires manual intervention".to_string(), notes))
    }

    /// Repair missing corresponding files
    async fn repair_missing_corresponding_file(&mut self, corruption: &CorruptionReport) -> Result<(bool, String, Vec<String>), ShardexError> {
        let mut notes = Vec::new();
        
        if corruption.description.contains("no corresponding vector storage") {
        ...
        if corruption.description.contains("no corresponding posting storage") {
        ...
        Ok((false, "Cannot determine missing file repair strategy".to_string(), notes))
    }

    /// Repair dimension mismatches
    async fn repair_dimension_mismatch(&mut self, _corruption: &CorruptionReport) -> Result<(bool, String, Vec<String>), ShardexError> {
        let notes = vec![
        ...
        Ok((false, "Dimension mismatch requires full index rebuild".to_string(), notes))
    }
}
...
mod tests {
    ...
    async fn test_integrity_checker_creation() {
        ...
        assert!(checker.component_paths.contains_key(&ComponentType::VectorStorage));
        assert!(checker.component_paths.contains_key(&ComponentType::PostingStorage));
    }
    ...
    async fn test_component_type_validation() {
        ...
        let vector_files = checker.component_paths.get(&ComponentType::VectorStorage).unwrap();
        assert!(!vector_files.is_empty());

        let posting_files = checker.component_paths.get(&ComponentType::PostingStorage).unwrap();
    ...
    async fn test_incremental_integrity_verification() {
        ...
        assert!(report.component_results.contains_key(&ComponentType::VectorStorage));
        assert!(!report.component_results.contains_key(&ComponentType::PostingStorage));
    }
    ...
    async fn test_cross_reference_validation() {
        ...
        let posting_path = temp_dir.path().join(format!("shards/{}.postings", shard_id));
        ...
        let posting_files = checker.component_paths.get(&ComponentType::PostingStorage).unwrap().clone();
        let vector_files = checker.component_paths.get(&ComponentType::VectorStorage).unwrap().clone();
        
        let issues = checker.verify_posting_vector_consistency(&posting_files, &vector_files).await.unwrap();
    ...
    async fn test_cross_reference_validation_missing_files() {
        ...
        assert!(issues[0].description.contains("no corresponding posting storage"));
    }
    ...
    async fn test_vector_quality_validation() {
        ...
        let result = checker.verify_vector_storage_file(&vector_path).await.unwrap();
    ...
    async fn test_repair_strategies() {
        ...
        let (success, action, notes) = checker.repair_vector_quality_issues(&nan_corruption).await.unwrap();
        ...
        let (success, action, notes) = checker.repair_count_mismatch(&count_mismatch).await.unwrap();
    ...
    async fn test_capacity_consistency_validation() {
        ...
        let result = checker.verify_header_compatibility(&posting_mmf, &vector_mmf).await;
