...
async fn test_concurrent_reader_writer_failures() {
    ...
    for writer_id in 0..3 {
        ...
        tasks.spawn(async move {
            for op_id in 0..10 {
                let result = concurrent_clone.write_operation(|_writer| {
    ...
    println!("Completed with {} successes and {} expected failures", 
...
async fn test_memory_pressure_behavior() {
        ...
        tasks.spawn(async move {
            ...
            for _op_id in 0..3 {
                ...
                if duration > Duration::from_millis(100) {
                    println!("Writer {} operation took {:?} (possible memory pressure)", 
                             writer_id, duration);
                }
    ...
    let monitor_task = tokio::spawn(async move {
        ...
        for _ in 0..10 {
            ...
            if stats.contended_writes > 0 {
                ...
                println!("Memory pressure indicator - contended writes: {}", 
    ...
    println!("Completed {} operations under memory pressure with {} contention observations", 
             completed_operations, contention_observations);
    
    // Verify system continued to function under pressure
    assert!(completed_operations > 0, "Should complete some operations even under pressure");
}
...
async fn test_cow_operation_crash_recovery() {
    ...
    // Get initial state
    let initial_shard_count = concurrent.read_operation(|index| {
        Ok(index.shard_count())
    ...
    let recovered_shard_count = recovered_concurrent.read_operation(|index| {
        Ok(index.shard_count())
    }).expect("Should be able to read recovered state");
    
    assert_eq!(initial_shard_count, recovered_shard_count, 
               "Recovered index should have same shard count as before crash");
    
    // Verify system can continue operating after recovery
    let post_recovery_result = recovered_concurrent.write_operation(|_writer| {
        Ok(42)
    }).await;
    
    assert!(post_recovery_result.is_ok(), "Should be able to perform operations after recovery");
    
    println!("Successfully recovered from simulated crash. Shard count: initial={}, recovered={}", 
...
async fn test_extreme_read_concurrency() {
    ...
    for writer_id in 0..5 {
        ...
        tasks.spawn(async move {
            for _op_id in 0..3 {
                let _result = concurrent_clone.write_operation(|_writer| {
                    Ok(writer_id)
                }).await;
    ...
    println!("Completed {} reads in {:?} with extreme concurrency", 
