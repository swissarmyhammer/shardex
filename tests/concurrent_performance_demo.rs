...
async fn test_concurrent_throughput_demonstration() {
    ...
    println!("Throughput: {:.1} ops/sec", total_successful as f64 / total_duration.as_secs_f64());
    
    let reader_successes: usize = results.iter()
        .filter(|(_, _, role)| *role == "reader")
        .map(|(_, successes, _)| *successes)
        .sum();
    
    let writer_successes: usize = results.iter()
    ...
    assert!(reader_successes > writer_successes,
...
async fn test_coordination_statistics() {
    ...
    let final_stats = concurrent.coordination_stats().expect("Failed to get final stats");
    assert_eq!(final_stats.total_writes, NUM_WRITES, "Should track all write operations");
    
    println!("Coordination statistics: total_writes={}, contention_rate={:.2}%", 
             final_stats.total_writes, final_stats.contention_rate());
}
...
async fn test_concurrent_readers_non_blocking() {
    ...
    assert!(total_duration < Duration::from_secs(1), 
           "Concurrent readers took too long (possible blocking): {:?}", total_duration);

    println!("Concurrent readers test: {} readers in {:?} (avg: {:.1}ms per reader)", 
