...
fn test_concurrent_error_scenarios() {
    ...
    let invalid_docs = vec![
        (DocumentId::new(), "This text is way too long for the small limit we have set"),
...
fn test_extraction_error_recovery() {
    ...
    for (start, length) in invalid_extractions {
        ...
        assert!(result.is_err(), "Expected error for extraction {}..{}", start, start + length);
    }
...
fn test_error_classification() {
    ...
    let size_error = storage.store_text_safe(DocumentId::new(), &"x".repeat(200)).unwrap_err();
    let not_found_error = storage.get_text_safe(DocumentId::new()).unwrap_err();
    let null_bytes_error = storage.store_text_safe(DocumentId::new(), "text\x00null").unwrap_err();
...
fn test_mixed_success_failure_patterns() {
    ...
    let operations = vec![
        ...
        (DocumentId::new(), "This text is definitely too long for the 50 character limit set in this test", false),
        (DocumentId::new(), "Valid 2", true),
        (DocumentId::new(), "Another text that clearly exceeds the configured 50 character limit for testing", false),
