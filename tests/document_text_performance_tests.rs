...
fn test_large_document_storage_performance() {
    ...
    for (word_count, description) in document_sizes {
        ...
        assert!(store_duration < config.large_document_timeout, 
            "Store time {:?} exceeded timeout {:?} for {}", 
            store_duration, config.large_document_timeout, description);
        ...
        assert!(retrieve_duration < config.large_document_timeout,
            "Retrieve time {:?} exceeded timeout {:?} for {}",
            retrieve_duration, config.large_document_timeout, description);
        ...
        for (start_pos, length) in extraction_positions {
            let start = Instant::now();
            let _substring = storage.extract_text_substring(
                doc_id, 
                start_pos as u32, 
                length as u32
            ).unwrap();
            ...
            assert!(extract_duration < config.extraction_timeout,
                "Extraction time {:?} exceeded timeout {:?} for {} at position {}",
                extract_duration, config.extraction_timeout, description, start_pos);
        }
...
fn test_many_documents_performance() {
    ...
    for document_count in document_counts {
        ...
        let document_text = format!("Performance test document with unique content for scalability testing.");
        ...
        println!("  Average store time per document: {:?}", 
            bulk_store_duration / document_count as u32);
        
        assert!(bulk_store_duration < config.many_documents_timeout,
            "Bulk store time {:?} exceeded timeout {:?} for {} documents",
            bulk_store_duration, config.many_documents_timeout, document_count);
        ...
        println!("  Random access time for {} samples: {:?}", sample_size, random_access_duration);
        println!("  Average random access time: {:?}", 
        ...
        println!("  Sequential access time for {} samples: {:?}", sample_size, sequential_duration);
        println!("  Average sequential access time: {:?}", 
    ...
    for (doc_id, text, doc_desc) in &doc_data {
        ...
        for (extract_size, size_desc) in &extraction_patterns {
            ...
            for position in positions {
                ...
                assert!(duration < config.extraction_timeout,
                    "Extraction time {:?} exceeded timeout {:?} for {} {} extraction",
                    duration, config.extraction_timeout, size_desc, doc_desc);
            }
            
            let avg_time = total_time / extraction_count as u32;
            println!("  {} extractions: avg {:?}, total {:?} for {} extractions",
...
fn test_file_growth_performance() {
    ...
    for (target_count, phase_desc) in growth_phases {
        println!("Testing {}: adding {} documents", phase_desc, target_count - all_docs.len());
        ...
        println!("  Random access time for {} samples: {:?}", sample_size, access_duration);
        println!("  Average access time: {:?}", access_duration / sample_size as u32);
        
        // Report storage statistics
        println!("  Total entries: {}", storage.entry_count());
        println!("  Total text size: {} bytes ({:.1} KB)", 
            storage.total_text_size(), 
            storage.total_text_size() as f64 / 1024.0);
        println!("  Utilization ratio: {:.2}%\n", storage.utilization_ratio() * 100.0);
        
        assert!(phase_duration < config.bulk_operations_timeout,
            "Phase duration {:?} exceeded timeout {:?} for {}",
            phase_duration, config.bulk_operations_timeout, phase_desc);
    }
}
...
fn test_memory_usage_scaling() {
    ...
    for (word_count, doc_count) in document_sizes {
        ...
        for _i in 0..doc_count {
            ...
            if text.len() >= 100 {
                let _extracted = storage.extract_text_substring(
                    doc_id, 
                    text.len() as u32 / 2, 
                    100
                ).unwrap();
...
fn test_update_performance() {
    ...
    println!("Testing update performance with {} documents, {} update cycles", doc_count, update_cycles);
    ...
    println!("Initial creation: {:?} ({:?} per document)", 
        creation_duration, creation_duration / doc_count as u32);
    ...
    for cycle in 0..update_cycles {
        ...
        for (i, &doc_id) in doc_ids.iter().enumerate() {
            let updated_text = format!("Updated content cycle {} for document {} with additional data.", cycle + 1, i);
        ...
        println!("Update cycle {}: {:?} ({:?} per document)", 
            cycle + 1, cycle_duration, cycle_duration / doc_count as u32);
        
        assert!(cycle_duration < config.many_documents_timeout,
            "Update cycle {} took {:?}, exceeded timeout {:?}",
            cycle + 1, cycle_duration, config.many_documents_timeout);
...
fn test_worst_case_performance() {
    ...
    println!("1000 updates to single document: {:?} ({:?} per update)",
        update_duration, update_duration / update_count);
    ...
    for i in 0..extraction_count {
        ...
        if length > 0 {
            let _extracted = storage.extract_text_substring(large_doc, position, length).unwrap();
        }
    }
    
    let fragmented_duration = extraction_start.elapsed();
    println!("{} fragmented extractions from large document: {:?} ({:?} per extraction)",
        extraction_count, fragmented_duration, fragmented_duration / extraction_count as u32);
    
    assert!(fragmented_duration < Duration::from_secs(1),
        "Fragmented extractions took too long: {:?}", fragmented_duration);
    ...
    println!("{} mixed operations: {:?} ({:?} per operation)",
        operations, mixed_duration, mixed_duration / operations as u32);
    
    assert!(mixed_duration < config.bulk_operations_timeout,
        "Mixed operations took too long: {:?}", mixed_duration);
