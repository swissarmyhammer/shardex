...
fn test_complete_document_storage_workflow() {
    ...
    for posting in &postings {
        ...
        assert_eq!(extracted, expected, 
            "Text mismatch for posting at {}..{}", expected_start, expected_end);
    }
}
...
fn test_document_replacement_workflow() {
    ...
    for posting in &original_postings {
        if posting.start as usize + posting.length as usize > updated_text.len() {
            let result = storage.extract_text_substring(
                posting.document_id,
                posting.start,
                posting.length,
            );
...
fn test_crash_recovery_simulation() {
    ...
    // Third session - add more data after recovery
    {
        ...
        // Add new document
        storage.store_text_safe(additional_doc, additional_text).unwrap();
...
fn test_large_document_integration() {
    ...
    for (i, posting) in postings.iter().enumerate() {
        ...
        assert_eq!(extracted, expected, "Mismatch for posting {} at {}..{}", 
    ...
    assert!(retrieval_duration.as_millis() < 100, 
        "Large document retrieval too slow: {:?}", retrieval_duration);
}
...
fn test_realistic_usage_workflow() {
    ...
    for (doc_id, text, postings) in &doc_data {
        ...
        for posting in postings {
            let snippet = storage.extract_text_substring(
                posting.document_id,
                posting.start,
                posting.length,
            ).unwrap();
