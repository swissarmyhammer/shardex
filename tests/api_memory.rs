...
use shardex::api::{
    AddPostings, AddPostingsParams, CreateIndex, CreateIndexParams, Flush, FlushParams,
    GetStats, GetStatsParams, Search, SearchParams, ShardexContext, StoreDocumentText,
...
fn test_context_lifecycle_management() {
    ...
    for iteration in 0..5 {
        ...
        println!("Iteration {}: Memory usage: {:.2} MB", 
                 iteration, stats.memory_usage as f64 / 1024.0 / 1024.0);
...
fn test_repeated_operations_memory_stability() {
    ...
    for batch in 0..10 {
        ...
        println!("Batch {}: Memory usage: {:.2} MB, Postings: {}", 
                 batch, 
        ...
        if batch > 0 {
            ...
            if batch > 2 { // Allow larger growth in first few batches
                assert!(growth_ratio < 0.5, 
                       "Memory growth of {:.2}% in batch {} suggests a memory leak", 
                       growth_ratio * 100.0, batch);
            }
        }
...
fn test_large_document_memory_handling() {
    ...
    for i in 0..10 {
        ...
        if i % 3 == 0 {
            ...
            println!("After {} large documents: {:.2} MB memory", 
                     i + 1,
                     stats.memory_usage as f64 / 1024.0 / 1024.0);
        }
    }
    ...
    println!("Final memory after large documents: {:.2} MB", 
...
fn test_error_handling_memory_safety() {
    ...
    let initial_stats = GetStats::execute(&mut context, &GetStatsParams::new())
        .expect("GetStats should succeed");
    
    println!("Initial memory usage: {:.2} MB", 
    ...
    for attempt in 0..5 {
        ...
        println!("After attempt {}: Memory usage: {:.2} MB", 
                 attempt, stats.memory_usage as f64 / 1024.0 / 1024.0);
...
fn test_memory_pressure_handling() {
    ...
    for batch in 0..50 {
        ...
        if total_postings % 100 == 0 {
            let flush_params = FlushParams::new();
            Flush::execute(&mut context, &flush_params)
                .expect("Flush should succeed to trigger shard creation");
        }
        ...
        if batch % 10 == 0 {
            ...
            println!("Batch {}: {} total postings, {:.2} MB memory, {} shards", 
                     batch, 
                     total_postings,
                     stats.memory_usage as f64 / 1024.0 / 1024.0,
                     stats.total_shards);
            ...
            assert!(memory_per_posting < 50000.0, // Less than 50KB per posting
        ...
        if batch % 20 == 19 {
            let flush_params = FlushParams::new();
            Flush::execute(&mut context, &flush_params)
                .expect("Flush should succeed under memory pressure");
        }
    }
    ...
    println!("Final: {} postings, {:.2} MB memory, {} shards", 
             total_postings,
             final_stats.memory_usage as f64 / 1024.0 / 1024.0,
    ...
    assert!(final_stats.memory_usage <= max_memory_usage * 2, 
