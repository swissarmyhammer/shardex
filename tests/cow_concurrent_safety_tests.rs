...
async fn test_reader_consistency_during_writes() {
    ...
    assert!(
        unique_counts.len() <= 6,
        "Too many different shard counts observed: {:?}",
        unique_counts
    );
    ...
    for &count in &snapshots {
        assert!(
            count >= initial_count,
            "Reader saw inconsistent shard count: {} < {}",
            count,
            initial_count
        );
    }
...
async fn test_sequential_writer_operations() {
    ...
    for i in 0..5 {
        ...
        assert_eq!(
            current_count,
            initial_count,
            "Shard count changed unexpectedly after write {}",
            i + 1
        );
    }
...
fn test_performance_overhead() {
    ...
    assert!(
        cow_per_op < 1_000_000, // < 1ms per operation
        "CoW read overhead too high: {} ns per operation",
        cow_per_op
    );
    assert!(
        quick_per_op < 100_000, // < 0.1ms per operation
        "Quick access overhead too high: {} ns per operation",
        quick_per_op
    );
    ...
    println!(
        "Performance test passed - CoW: {} ns/op, Quick: {} ns/op",
