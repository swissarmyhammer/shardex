...
fn test_text_index_header_operations() {
    ...
    assert_eq!(header.next_entry_offset, TextIndexHeader::SIZE as u64 + DocumentTextEntry::SIZE as u64);
    assert!(!header.is_empty());
    assert_eq!(header.total_entries_size(), DocumentTextEntry::SIZE as u64);
    
    header.add_entry();
    assert_eq!(header.entry_count, 2);
    assert_eq!(header.next_entry_offset, TextIndexHeader::SIZE as u64 + 2 * DocumentTextEntry::SIZE as u64);
...
fn test_text_index_header_offset_calculations() {
    ...
    assert_eq!(header.offset_for_entry(1), TextIndexHeader::SIZE as u64 + DocumentTextEntry::SIZE as u64);
    assert_eq!(header.offset_for_entry(10), TextIndexHeader::SIZE as u64 + 10 * DocumentTextEntry::SIZE as u64);
...
fn test_text_data_header_operations() {
    ...
    assert!(new_utilization > 0.0 && new_utilization <= 1.0, 
        "New utilization {} should be between 0.0 and 1.0", new_utilization);
}
...
fn test_header_version_validation() {
    ...
    assert!(index_header.file_header.validate_version(TEXT_INDEX_VERSION, TEXT_INDEX_VERSION).is_ok());
    assert!(data_header.file_header.validate_version(TEXT_DATA_VERSION, TEXT_DATA_VERSION).is_ok());
    
    // Should reject versions outside valid range  
    assert!(index_header.file_header.validate_version(TEXT_INDEX_VERSION + 1, TEXT_INDEX_VERSION + 2).is_err());
    assert!(data_header.file_header.validate_version(TEXT_DATA_VERSION + 1, TEXT_DATA_VERSION + 2).is_err());
}
...
fn test_comprehensive_entry_validation_edge_cases() {
    ...
    for (offset, length, should_fail) in test_cases {
        ...
        if should_fail {
            assert!(result.is_err(), "Expected validation failure for offset={}, length={}", offset, length);
        } else {
            assert!(result.is_ok(), "Expected validation success for offset={}, length={}", offset, length);
        }
    }
}