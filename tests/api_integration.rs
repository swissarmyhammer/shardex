...
use shardex::api::{
    AddPostings, AddPostingsParams, BatchStoreDocumentText, BatchStoreDocumentTextParams, CreateIndex,
    CreateIndexParams, DocumentTextEntry, ExtractSnippet, ExtractSnippetParams, Flush, FlushParams, GetDocumentText,
...
fn test_api_basic_operations_integration() {
    ...
    let add_params = AddPostingsParams::new(test_postings.clone()).expect("Failed to create AddPostingsParams");

    AddPostings::execute(&mut context, &add_params).expect("AddPostings should succeed");

    // Test Flush operation
    let flush_params = FlushParams::with_stats();
    let flush_result = Flush::execute(&mut context, &flush_params).expect("Flush should succeed");
    ...
    let search_results = Search::execute(&mut context, &search_params).expect("Search should succeed");
    ...
    let stats = GetStats::execute(&mut context, &stats_params).expect("GetStats should succeed");
...
fn test_document_text_integration() {
    ...
    let retrieved_text = GetDocumentText::execute(&mut context, &get_params).expect("GetDocumentText should succeed");
    ...
    let snippet = ExtractSnippet::execute(&mut context, &extract_params).expect("ExtractSnippet should succeed");
    ...
    let found_text = GetDocumentText::execute(
        &mut context,
...
fn test_batch_operations_integration() {
    ...
    let batch_stats =
        BatchStoreDocumentText::execute(&mut context, &batch_params).expect("BatchStoreDocumentText should succeed");
    ...
    for doc_entry in &batch_data {
        let retrieved_text = GetDocumentText::execute(
            &mut context,
            &GetDocumentTextParams {
    ...
    for result in &results {
        assert!(
            result.document_id.raw() >= 1 && result.document_id.raw() <= 5,
...
fn test_operations_workflow_integration() {
    ...
    let add_params = AddPostingsParams::new(initial_postings).expect("Failed to create AddPostingsParams");
    ...
    let text_documents = vec![
        (
            DocumentId::from_raw(21),
            "Research paper on artificial intelligence applications",
        ),
        (
            DocumentId::from_raw(22),
            "Study on machine learning algorithms and optimization",
        ),
        (
            DocumentId::from_raw(23),
            "Analysis of deep learning techniques in computer vision",
        ),
    ];
    ...
    let flush_params = FlushParams::with_stats();
    let flush_stats = Flush::execute(&mut context, &flush_params).expect("Flush should succeed");
    ...
    for result in &results {
        if result.document_id.raw() >= 21 && result.document_id.raw() <= 23 {
            let text = GetDocumentText::execute(
                &mut context,
                &GetDocumentTextParams {
            ...
            assert!(
                text.contains("artificial") || text.contains("machine") || text.contains("deep"),
