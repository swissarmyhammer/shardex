...
fn test_unicode_text_handling() {
    ...
    let test_cases = vec![
        ...
        ("Emoji", "Hello 🌍 World! 🚀🎉"),
        ("Mixed content", "Hello 世界! Здравствуй мир! 🌍 Español Français العربية русский"),
    ];
...
fn test_size_limit_enforcement() {
    ...
    storage.store_text(under_limit_doc, &under_limit_text).unwrap();
...
fn test_large_document_handling() {
    let temp_dir = TempDir::new().unwrap();
    let large_limit = 10 * 1024 * 1024; // 10MB limit
...
fn test_concurrent_document_operations() {
    ...
    for i in 0..doc_ids.len() {
        ...
        if i >= 2 {
            ...
            storage.store_text(doc_ids[update_idx], &updated_text).unwrap();
...
fn test_storage_statistics() {
    ...
    assert_eq!(storage.total_text_size(), (text1.len() + text2.len() + text1_updated.len()) as u64);
}
...
fn test_boundary_text_lengths() {
    ...
    assert!(storage.store_text(over_limit_doc, &over_limit_text).is_err());
}
