...
use shardex::{
    CrashRecovery, Posting, Shard, ShardexConfig, ShardexIndex,
    WalSegment, WalTransaction, WalOperation, DocumentId, TransactionId, ShardId, DirectoryLayout
};
...
async fn test_recovery_performance() {
    ...
    assert!(detection_time < Duration::from_millis(1000), 
        "Crash detection took too long: {:?}", detection_time);
...
async fn test_recovery_with_various_configs() {
    ...
    for vector_size in [64, 128, 256, 512] {
        ...
        assert!(!crash_detected, "Clean index should not show crash for vector_size={}", vector_size);
    }
}
...
async fn test_acceptance_criteria_validation() {
    ...
    assert!(stats.total_recovery_duration < Duration::from_secs(10), 
